...
namespace Roblox.Common
{
	public static class Extensions
	{
		...
		{
			return text.ParsedSubString(startToken, endToken, false, false, startWithLastOccurrence);
		}
		...
		{
			string defaultOp = string.Format(" {0} ", defaultOperator);
			string searchQuery = "";
			if (!string.IsNullOrEmpty(query.Trim()))
			{
				...
				for (int j = 1; j < searchTerms.Count; j++)
				{
					...
					searchQuery = searchQuery + " " + searchTerms[j];
				}
		...
		private static void AddSearchTerm(string searchTerm, ref List<string> searchTerms, QueryOperators defaultOperator)
		{
			...
			searchTerm = Regex.Replace(searchTerm, "^(\\-{1,}|\\!{1,})", string.Format(" {0} not ", defaultOperator));
		...
		{
			T result;
			if (Enum.TryParse<T>(value, ignoreCase, out result) && Enum.IsDefined(typeof(T), result))
			{
				return result;
			}
			return null;
		...
		{
			return first.Intersect(second, (IEqualityComparer<TSource>)new LambdaComparer<TSource>(comparer));
		}
