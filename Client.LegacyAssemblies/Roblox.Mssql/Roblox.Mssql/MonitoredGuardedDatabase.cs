...
namespace Roblox.Mssql
{
	public class MonitoredGuardedDatabase : GuardedDatabase
	{
		...
		private struct _003CStartMonitoringAsync_003Ed__10 : IAsyncStateMachine
		{
			...
			public MonitoredGuardedDatabase _003C_003E4__this;
			...
			private void MoveNext()
			{
				...
				MonitoredGuardedDatabase monitoredGuardedDatabase = _003C_003E4__this;
				try
				{
					if (num == 0)
					{
					...
					if (monitoredGuardedDatabase._IsDisposed)
					{
						throw new ObjectDisposedException(monitoredGuardedDatabase.GetType().ToString());
					}
					if (!monitoredGuardedDatabase._IsMonitoring)
					{
						goto IL_0038;
					}
					goto end_IL_000e;
					...
					try
					{
						...
						if (num != 0)
						{
							awaiter = monitoredGuardedDatabase._Semaphore.WaitAsync(cancellationToken).ConfigureAwait(false).GetAwaiter();
						...
						if (!monitoredGuardedDatabase._IsMonitoring)
						{
							monitoredGuardedDatabase._IsMonitoring = true;
							goto IL_00ce;
						}
					}
					finally
					{
						if (num < 0)
						{
							monitoredGuardedDatabase._Semaphore.Release();
						}
					}
					goto end_IL_000e;
					IL_00ce:
					monitoredGuardedDatabase._DatabaseObserver = monitoredGuardedDatabase._DatabaseObserverBuilder(monitoredGuardedDatabase);
					monitoredGuardedDatabase._DatabaseObserver.Register();
					end_IL_000e:;
				}
		...
		private struct _003CStopMonitoringAsync_003Ed__12 : IAsyncStateMachine
		{
			...
			private void MoveNext()
			{
				...
				try
				{
					if (num == 0 || monitoredGuardedDatabase._IsMonitoring)
					{
						try
						{
							ConfiguredTaskAwaitable.ConfiguredTaskAwaiter awaiter;
							...
							if (monitoredGuardedDatabase._IsMonitoring)
							{
								monitoredGuardedDatabase._IsMonitoring = false;
								goto IL_00b5;
							}
						}
						finally
					...
					goto end_IL_000e;
					IL_00b5:
					monitoredGuardedDatabase._DatabaseObserver.Dispose();
					monitoredGuardedDatabase._DatabaseObserver = null;
					end_IL_000e:;
				}
				catch (Exception exception)
				{
