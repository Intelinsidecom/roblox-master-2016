...
namespace Roblox.Platform.Infrastructure
{
	internal class ServerValidatorInstance : IServerValidator
	{
		...
		{
			Server server;
			if (!IsIpAddressInIpToServerMap(ipAddress, out server))
			{
				return null;
			}
		...
		public bool IsAllowedProxyIp(string ipAddressString)
		{
			...
			Server server;
			if (!IsIpAddressInIpToServerMap(ipAddressString, out server))
			{
				return false;
			}
		...
		private bool IsIpAddressInRange(IReadOnlyCollection<IpAddressRange> ipRanges, IPAddress ipAddress)
		{
			...
			return ipRanges.Aggregate(false, (bool result, IpAddressRange ipRange) => result || ipRange.IsInRange(ipAddress));
		}
	}
}
