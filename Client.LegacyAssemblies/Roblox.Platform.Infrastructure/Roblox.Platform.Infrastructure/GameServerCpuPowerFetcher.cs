...
namespace Roblox.Platform.Infrastructure
{
	public class GameServerCpuPowerFetcher
	{
		...
		{
			int cpuPower;
			_GamesRelayIpAddressToCpuPowerMap.Value.TryGetValue(ipAddress, out cpuPower);
			return cpuPower;
		}
		...
		private IDictionary<string, int> GetServerIpAddressToCpuPowerMap(int serverGroupId, int environmentId)
		{
			...
			foreach (IDictionary<string, object> item in ((RobloxDatabase)94).ExecuteReader("Servers_GetRccServerIpAddressDataCenterIDTotalCPUPowerByServerGroupIDAndEnvironmentID", parameters, CommandType.StoredProcedure, (int?)null, (ApplicationIntent?)null))
			{
