...
namespace Roblox.Redis
{
	public static class RedisPagedOperator
	{
		public static void DeleteSortedSetEntriesPaged(this IRedisClient redisClient, string key, double inclusiveMinScore, double inclusiveMaxScore, int pageSize, CommandFlags deletionCommandFlags)
		{
			...
			{
				redisClient.Execute(key, (IDatabase db) => db.SortedSetRemoveRangeByScore(key, range.Item1, range.Item2, (Exclude)0, deletionCommandFlags));
			}
		}
		...
		private static IReadOnlyCollection<Tuple<double, double>> GetSortedSetRanges(this IRedisClient redisClient, string key, double inclusiveMinScore, double inclusiveMaxScore, int pageSize)
		{
			...
			do
			{
				...
				if (!num2.HasValue)
				{
					array = redisClient.Execute(key, (IDatabase db) => db.SortedSetRangeByScoreWithScores(key, inclusiveMinScore, stopScoreToFetchUpto, (Exclude)0, (Order)0, startIndex, 1L, (CommandFlags)0));
				...
				array = redisClient.Execute(key, (IDatabase db) => db.SortedSetRangeByScoreWithScores(key, inclusiveMinScore, stopScoreToFetchUpto, (Exclude)0, (Order)0, startIndex + pageSize, 1L, (CommandFlags)0));
