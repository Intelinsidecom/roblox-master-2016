...
namespace Roblox.Caching.Shared
{
	internal class RolloutPerThousandSharedCacheClient : DynamicSharedCacheClientBase
	{
		...
		protected override ISharedCacheClient Client
		{
			get
			{
				return GetCacheClientToUseForRollout();
			}
		}
		...
		public RolloutPerThousandSharedCacheClient(ISharedCacheClient sourceCacheClient, ISharedCacheClient destinationCacheClient, Func<int> rolloutPerThousandGetter)
		{
			...
			_SourceCacheClient = sourceCacheClient;
			if (destinationCacheClient == null)
			{
				throw new ArgumentNullException("destinationCacheClient");
			}
			_DestinationCacheClient = destinationCacheClient;
			if (rolloutPerThousandGetter == null)
			{
				throw new ArgumentNullException("rolloutPerThousandGetter");
			}
			_RolloutPerThousandGetter = rolloutPerThousandGetter;
		}
