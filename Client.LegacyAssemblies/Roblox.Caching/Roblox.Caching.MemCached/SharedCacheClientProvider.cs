...
namespace Roblox.Caching.MemCached
{
	public class SharedCacheClientProvider : ISharedCacheClientProvider
	{
		...
		internal SharedCacheClientProvider(ISettings settings, ISharedCacheClient fallbackCache, IMemcachedGroupCacheClientProvider memcachedGroupCacheClientProvider, IRolloutPerThousandSharedCacheClientProvider rolloutPerThousandSharedCacheClientProvider, IMigrationStateMonitor migrationStateMonitor, ILogger logger)
		{
			...
			{
				throw new ArgumentNullException("fallbackCache");
			}
			_DefaultCacheClient = fallbackCache;
			if (memcachedGroupCacheClientProvider == null)
			{
				throw new ArgumentNullException("memcachedGroupCacheClientProvider");
			}
			_MemcachedGroupCacheClientProvider = memcachedGroupCacheClientProvider;
			if (rolloutPerThousandSharedCacheClientProvider == null)
			{
				throw new ArgumentNullException("rolloutPerThousandSharedCacheClientProvider");
			}
			_RolloutPerThousandSharedCacheClientProvider = rolloutPerThousandSharedCacheClientProvider;
			if (migrationStateMonitor == null)
			{
				throw new ArgumentNullException("migrationStateMonitor");
			}
			_MigrationStateMonitor = migrationStateMonitor;
			if (logger == null)
			{
		...
		public ISharedCacheClient GetCacheClientForSettings(IRemoteCachabilitySettings remoteCachabilitySettings, IMigrationCacheabilitySettings migrationCacheabilitySettings, string metricsIdentifier)
		{
			...
			if (!string.IsNullOrWhiteSpace((migrationCacheabilitySettings != null) ? migrationCacheabilitySettings.MigrationMemcachedGroupName : null) && !(migrationCacheabilitySettings.MigrationStateChange == null))
			{
				MigrationStateChange migrationStateChange = migrationCacheabilitySettings.MigrationStateChange;
				if ((object)migrationStateChange == null || migrationStateChange.SourceState != MigrationState.NoMigration || !migrationCacheabilitySettings.MigrationStateChange.IsSourceAndDestinationStateSame)
				{
					...
					if ((migrationStateChange2.SourceState != MigrationState.NoMigration || migrationStateChange2.DestinationState != MigrationState.NoMigration) && string.IsNullOrWhiteSpace(migrationCacheabilitySettings.MigrationMemcachedGroupName))
					{
						if (_Settings.IsLoggingMigrationConfigurationErrorsEnabled)
						{
							_Logger.Error(string.Format("MetricsIdentifier = {0}: MigrationCacheabilitySettings.MigrationStateChange was set to {1} for group name {2} but MigrationCacheabilitySettings.MigrationMemcachedGroupName was null or whitespace.", metricsIdentifier, migrationStateChange2, remoteCachabilitySettings.MemcachedGroupName));
						}
