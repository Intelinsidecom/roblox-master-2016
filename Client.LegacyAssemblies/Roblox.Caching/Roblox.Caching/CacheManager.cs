...
namespace Roblox.Caching
{
	public class CacheManager
	{
		public class CachePolicy
		{
			...
			public DateTime UtcExpirationDate
			{
				get
				{
		...
		public static TimeSpan RemoteCacheableExpiration
		{
			get
			{
				return Settings.Default.RemoteCacheableExpiration;
			}
		}
		...
		public static LocalCache LocalCache
		{
			get
			{
				return CacheFactory.GetLocalCache();
			}
		}
		...
		public static IEnumerable<ValueTuple<TIndex, TEntity, bool>> TryGetEntitiesOnlyFromCache<TIndex, TEntity>(ICacheInfo cacheInfo, IReadOnlyCollection<TIndex> entityIds) where TEntity : class, ICacheableObject<TIndex>
		{
			if (cacheInfo == null)
			{
		...
		public static ICollection<T> GetIDCollectionFromCache<T>(ICacheInfo cacheInfo, SortedDictionary<T, string> lookupToCacheKeyLookupMap, Func<ICollection<T>, ICollection<T>> getterByLookUps) where T : struct
		{
			...
			{
				T entityId;
				if (cacheForEntity.GetEntityIDFromLookupCache<T>(cacheInfo, item.Value, out entityId))
				{
					list.Add(entityId);
					continue;
				}
			...
			for (int i = 0; i < list.Count; i++)
			{
				...
				{
					list[i] = collection.ElementAt(num);
					num++;
				}
			}
			return list.Where((T x) => !x.Equals(default(T))).ToList();
		}
