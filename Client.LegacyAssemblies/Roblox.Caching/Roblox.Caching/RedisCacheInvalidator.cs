...
namespace Roblox.Caching
{
	internal class RedisCacheInvalidator : ICacheInvalidator
	{
		...
		public void RemoveRemoteKeyRaw(string listenerTopic, string key, string entityType)
		{
			...
			RedisPublisher publisher = _Publisher;
			if (publisher != null)
			{
				publisher.PublishMessage(message, entityType);
			}
		}
		...
		private void HandleMessageReceived(string messageText)
		{
			...
			IRawMessageConsumer rawMessageConsumer = _RawMessageConsumer;
			if (rawMessageConsumer != null)
			{
				rawMessageConsumer.ReadMessage(text, key);
			}
			...
			IRedisInvalidationPerformanceMonitor redisInvalidationPerformanceMonitor = _RedisInvalidationPerformanceMonitor;
			if (redisInvalidationPerformanceMonitor != null)
			{
				redisInvalidationPerformanceMonitor.MessagesReceivedPerSecond.Increment();
			}
			IRedisInvalidationPerformanceMonitor redisInvalidationPerformanceMonitor2 = _RedisInvalidationPerformanceMonitor;
			if (redisInvalidationPerformanceMonitor2 != null)
			{
				redisInvalidationPerformanceMonitor2.TotalMessagesReceived.Increment();
			}
		}
		...
		private void OnPublishCompleted(string message)
		{
			...
			if (redisInvalidationPerformanceMonitor != null)
			{
				redisInvalidationPerformanceMonitor.MessagesPublishedPerSecond.Increment();
			}
			IRedisInvalidationPerformanceMonitor redisInvalidationPerformanceMonitor2 = _RedisInvalidationPerformanceMonitor;
			if (redisInvalidationPerformanceMonitor2 != null)
			{
				redisInvalidationPerformanceMonitor2.TotalMessagesPublished.Increment();
			}
		}
		...
		private void OnPubSubHandlesRemoved(IReadOnlyCollection<ISubscriber> handles)
		{
			...
			if (publisher != null)
			{
				publisher.OnPubSubHandlesRemoved(handles);
			}
			Task.Delay(_RedisInvalidationSettingsProvider.GetDelayToStopSubscribingAfterNodeRemoval()).ContinueWith(delegate
			{
				RedisSubscriber subscriber = _Subscriber;
				if (subscriber != null)
				{
					subscriber.OnPubSubHandlesRemoved(handles);
				}
			});
		}
		...
		private void OnPubSubHandlesAdded(IReadOnlyCollection<ISubscriber> handles)
		{
			...
			if (subscriber != null)
			{
				subscriber.OnPubSubHandlesAdded(handles, registeredEntities);
			}
			Task.Delay(_RedisInvalidationSettingsProvider.GetDelayToStartPublishingAfterNodeAddition()).ContinueWith(delegate
			{
				...
				if (publisher != null)
				{
					publisher.OnPubSubHandlesAdded(handles);
				}
			});
		}
	}
