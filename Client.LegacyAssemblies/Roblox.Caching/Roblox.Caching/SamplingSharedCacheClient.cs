...
namespace Roblox.Caching
{
	internal sealed class SamplingSharedCacheClient : ISharedCacheClient, IDisposable
	{
		private sealed class InfluxDbSampleLogger : IDisposable
		{
			...
			{
				[CompilerGenerated]
				private readonly LogItemType _003CType_003Ek__BackingField;

				[CompilerGenerated]
				private readonly string _003CStackTrace_003Ek__BackingField;
				...
				public string StackTrace
				{
					...
					get
					{
						return _003CStackTrace_003Ek__BackingField;
					}
				}
				...
				public LogItem(LogItemType type, string stackTrace, int? size, DateTime timeStamp)
				{
					...
					_003CStackTrace_003Ek__BackingField = stackTrace;
					_003CSize_003Ek__BackingField = size;
					_003CTimeStamp_003Ek__BackingField = timeStamp;
				}
			}
			...
			{
				if (backingSharedCacheClient == null)
				{
					throw new ArgumentNullException("backingSharedCacheClient");
				}
				_BackingSharedCacheClient = backingSharedCacheClient;
				if (logger == null)
				{
				...
				_LogItemsEvent = new AutoResetEvent(false);
			...
			{
				while (!_LoggingTaskCancellationTokenSource.IsCancellationRequested || _LogItems.Any())
				{
					try
					{
			...
			private void WriteEntries(IReadOnlyCollection<LogItem> logItems)
			{
				...
				influxUrl += string.Format("/write?db=${0}", _Settings.InfluxDatabase);
				...
				foreach (LogItem logItem in logItems)
				{
					...
					if (!hashSet.Contains(text))
					{
						try
						{
							_BackingSharedCacheClient.Set(string.Format("MemcachedUsageStackTraceHash:{0}", text), logItem.StackTrace, _Settings.SampleLogExpiration);
							hashSet.Add(text);
						}
						catch (Exception ex)
						{
					...
					stringBuilder.Append(string.Format("{0}s,stackTraceHash={1},knownSize={2} size={3} {4}", logItem.Type, text, logItem.Size.HasValue, logItem.Size ?? 1, num));
				}
				try
				{
		...
		{
			if (backingCacheClient == null)
			{
				throw new ArgumentNullException("backingCacheClient");
			}
			_BackingCacheClient = backingCacheClient;
			if (logger == null)
			{
		...
		{
			_SampleLogger.LogBytesWritten((value != null) ? new int?(value.Length) : ((int?)null));
		...
		{
			_SampleLogger.LogBytesRead(8 * ((keys != null) ? new int?(keys.Length) : ((int?)null)));
		...
		public string[] MultiGetAsStrings(string[] keys)
		{
			...
			_SampleLogger.LogBytesRead(((IEnumerable<string>)array).Sum((Func<string, int>)GetStringSize));
			return array;
		}
		...
		public bool QueryBytes(string key, out byte[] value)
		{
			...
			InfluxDbSampleLogger sampleLogger = _SampleLogger;
			byte[] obj = value;
			sampleLogger.LogBytesRead((obj != null) ? new int?(obj.Length) : ((int?)null));
			return result;
		}
