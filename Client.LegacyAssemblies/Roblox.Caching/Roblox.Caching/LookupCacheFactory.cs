...
namespace Roblox.Caching
{
	public class LookupCacheFactory : ILookupCacheFactory
	{
		...
		internal LookupCacheFactory(ISettings settings, ILogger logger, ISharedCacheClientProvider cacheClientProvider, ICounterRegistry counterRegistry)
		{
			...
			_Logger = logger;
			if (cacheClientProvider == null)
			{
				throw new ArgumentNullException("cacheClientProvider");
			}
			_CacheClientProvider = cacheClientProvider;
		...
		public ILookupCache GetLookupCache(ICacheInfo cacheInfo)
		{
			...
			IRemoteCachabilitySettings remoteCachabilitySettings = cacheInfo.RemoteCachabilitySettings;
			string memcachedGroupNameOrDefault = GetMemcachedGroupNameOrDefault((remoteCachabilitySettings != null) ? remoteCachabilitySettings.MemcachedGroupName : null);
