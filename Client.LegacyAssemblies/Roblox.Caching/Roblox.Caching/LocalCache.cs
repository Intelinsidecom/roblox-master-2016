...
namespace Roblox.Caching
{
	public abstract class LocalCache : RobloxCache, IRemoteCacheInvalidationSink
	{
		...
		[CompilerGenerated]
		private readonly string _003CNodeId_003Ek__BackingField = Guid.NewGuid().ToString("N").Substring(8);

		[CompilerGenerated]
		private readonly string _003CTopic_003Ek__BackingField;
		...
		public string NodeId
		{
			...
			get
			{
				return _003CNodeId_003Ek__BackingField;
			}
		}
		...
		public string Topic
		{
			...
			get
			{
				return _003CTopic_003Ek__BackingField;
			}
		}
		...
		public static TimeSpan GlobalSlidingExpiration
		{
			get
			{
		...
		protected LocalCache(ICounterRegistry counterRegistry, bool replicate)
		{
			...
			RequestCount = _CounterRegistry.GetRateOfCountsPerSecondCounter("Roblox Cache Replication (CMN)", "Request Count / sec", (string)null);
			InsertCount = _CounterRegistry.GetRateOfCountsPerSecondCounter("Roblox Cache Replication (CMN)", "Insert Count / sec", (string)null);
			LocalRemoveCount = _CounterRegistry.GetRateOfCountsPerSecondCounter("Roblox Cache Replication (CMN)", "Local Count / sec", (string)null);
			RemoteRemoveCount = _CounterRegistry.GetRateOfCountsPerSecondCounter("Roblox Cache Replication (CMN)", "Remote Count / sec", (string)null);
			_Replicate = replicate;
			_003CTopic_003Ek__BackingField = "RBX-" + TopicNamespace + NodeId;
			if (_Replicate)
			{
		...
		private T FillItemFetcher<T>(ICacheInfo cacheInfo, CacheManager.GetCount<T> getter, string key, ItemFetcher itemFetcher) where T : struct
		{
			...
			lock (itemFetcher)
			{
				...
				t = getter(ref policy);
				string text = ((policy.ScopeFilter != CacheManager.CacheScopeFilter.Unqualified) ? Cacheable.BuildStateTokenKey(cacheInfo, policy.ScopeFilter, policy.CachedStateQualifier) : Cacheable.BuildStateTokenKey(cacheInfo, policy.ScopeFilter, null));
		...
		[return: TupleElementNames(new string[] { "Key", "Value", "CacheHit" })]
		private IEnumerable<ValueTuple<string, T, bool>> GetValues<T>(IReadOnlyCollection<string> keys)
		{
			RequestCount.Increment();
			IEnumerable<ValueTuple<string, object>> enumerable = RawGetValues(keys);
			foreach (var item3 in enumerable)
			{
				...
				if (item2 == null)
				{
					yield return new ValueTuple<string, T, bool>(item, default(T), false);
				}
				else if (item2 is NullMarker)
				{
					yield return new ValueTuple<string, T, bool>(item, default(T), true);
				}
				else
				{
					yield return new ValueTuple<string, T, bool>(item, (T)item2, true);
				}
			}
		}
		...
		private void PerformItemFetchersCleanUp(bool createdCachedObject, string key)
		{
			...
			{
				ItemFetcher value;
				_ItemFetchers.TryRemove(key, out value);
			}
		}
		...
		private void InvalidateRemoteCaches<T>(ICacheableObject<T> entity)
		{
			...
			{
				string key = Cacheable.BuildEntityIDLookupKey(cacheInfo, item);
				EntityCacheInvalidator.RemoveRemoteKey(this, cacheInfo.EntityType, key);
			}
		}
		...
		protected abstract IEnumerable<ValueTuple<string, object>> RawGetValues(IReadOnlyCollection<string> keys);
		...
		public void AddEntityIDCollectionToCache<T>(CacheInfo cacheInfo, string collectionId, ICollection<T> collection, CacheManager.CacheScopeFilter collectionScope, string cachedStateQualifier) where T : struct
		{
			...
			{
				string text = ((collectionScope != CacheManager.CacheScopeFilter.Unqualified) ? Cacheable.BuildStateTokenKey(cacheInfo, collectionScope, cachedStateQualifier) : Cacheable.BuildStateTokenKey(cacheInfo, collectionScope, null));
				AddStateTokenToCache(text);
				string[] dependency = new string[1] { text };
		...
		public override TEntity TryGetEntityOnlyFromCache<TIndex, TEntity>(ICacheInfo cacheInfo, TIndex entityId, out bool cacheHit)
		{
			...
			string key = Cacheable.BuildEntityKey(cacheInfo, entityId);
			TEntity result;
			if (GetValue<TEntity>(key, out result))
			{
				cacheHit = true;
		...
		public override IEnumerable<ValueTuple<TIndex, TEntity, bool>> TryGetEntitiesOnlyFromCache<TIndex, TEntity>(ICacheInfo cacheInfo, IReadOnlyCollection<TIndex> entityIds)
		{
			...
			Dictionary<string, TIndex> dictionary = entityIds.ToDictionary((TIndex id) => Cacheable.BuildEntityKey(cacheInfo, id), (TIndex id) => id);
			IEnumerable<ValueTuple<string, TEntity, bool>> values = GetValues<TEntity>(dictionary.Keys);
			ValueTuple<TIndex, TEntity, bool>[] array = new ValueTuple<TIndex, TEntity, bool>[entityIds.Count];
			int num = 0;
			int num2 = 0;
			foreach (var item4 in values)
			{
				string item = item4.Item1;
				TEntity item2 = item4.Item2;
				bool item3 = item4.Item3;
				if (item3)
				{
					num2++;
				}
				array[num++] = new ValueTuple<TIndex, TEntity, bool>(dictionary[item], item2, item3);
			}
			if (num2 > 0)
			{
		...
		public override TEntity TryGetEntityOnlyFromCacheByIDLookup<TIndex, TEntity>(ICacheInfo cacheInfo, string entityIdLookup, out bool cacheHit)
		{
			...
			TIndex entityId;
			cacheHit = CacheManager.GetEntityIDFromLookupCache<TIndex>(cacheInfo, entityIdLookup, out entityId);
			if (!cacheHit)
			{
				return null;
			}
		...
		public override TEntity GetEntityFromCacheByIDLookup<TIndex, TEntity>(ICacheInfo cacheInfo, string entityIdLookup, Func<TEntity> getter)
		{
			...
			TIndex entityId;
			if (!CacheManager.GetEntityIDFromLookupCache<TIndex>(cacheInfo, entityIdLookup, out entityId))
			{
				return getter();
			}
			if (entityId.CompareTo(default(TIndex)) != 0)
		...
		public ICollection<T> GetIDCollectionFromCache<T>(ICacheInfo cacheInfo, string collectionId, CacheManager.GetIDCollection<T> getter) where T : struct
		{
			...
			if (GetValue<ICollection<T>>(key, out result))
			{
				return result;
			}
			bool createdCachedObject = false;
