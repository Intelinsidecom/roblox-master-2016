...
namespace Roblox.Caching
{
	...
	public class MigrationStateChange : IEquatable<MigrationStateChange>
	{
		...
		public bool IsSourceAndDestinationStateSame
		{
			get
			{
		...
		public MigrationStateChange(string migrationStateChange)
		{
			...
			MigrationState? sourceStateNullable = EnumUtils.StrictTryParseEnum<MigrationState>(array[0]);
			if (!sourceStateNullable.HasValue)
			{
				throw new MigrationStateChangeParseException(string.Format("Could not resolve SourceState. Value was {0}.", array[0]));
			}
			SourceState = sourceStateNullable.Value;

			MigrationState? destinationStateNullable = EnumUtils.StrictTryParseEnum<MigrationState>(array[1]);
			if (!destinationStateNullable.HasValue)
			{
				throw new MigrationStateChangeParseException(string.Format("Could not resolve DestinationState. Value was {0}.", array[1]));
			}
			DestinationState = destinationStateNullable.Value;
			int result;
			if (!int.TryParse(array[2], out result))
			{
				throw new MigrationStateChangeParseException(string.Format("Could not resolve Rollout Per Thousand. Value was {0}", array[2]));
			}
			...
			throw new MigrationStateChangeParseException(string.Format("Rollout Per Thousand was out of range. Value was {0}", array[2]));
		}
