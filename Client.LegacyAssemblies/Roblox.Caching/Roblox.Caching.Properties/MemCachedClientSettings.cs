...
namespace Roblox.Caching.Properties
{
	...
	public class MemCachedClientSettings : IMemCachedClientSettings, IMemcachedClientDnsSettings, INotifyPropertyChanged
	{
		...
		public int MaximumNumberOfSocketsPerPool
		{
			get
			{
				Func<int> maximumNumberOfSocketsPerPoolOverride = MaximumNumberOfSocketsPerPoolOverride;
				if (maximumNumberOfSocketsPerPoolOverride == null)
				{
					return _Settings.MemcachedMaximumNumberOfSocketsPerPool;
				}
				return maximumNumberOfSocketsPerPoolOverride();
			}
		}
		...
		public IMemcachedClientDnsSettings DnsSettings
		{
			get
			{
				return (IMemcachedClientDnsSettings)(object)this;
			}
		}
		...
		internal Dictionary<string, TimeSpan> ParseExpiryOverrides(string csv)
		{
			...
			try
			{
				...
				foreach (string text in array)
				{
					...
					if (array2.Length != 2)
					{
						_Logger.Error(string.Format("Invalid pair found in ExpiryOverride setting, expected host and expiry to be separated by equals sign: '{0}'", text));
						continue;
					}
					...
					{
						_Logger.Error(string.Format("Invalid pair found in ExpiryOverride setting, invalid host: '{0}'", text2));
					}
					else if (!TimeSpan.TryParse(text3, out result))
					{
						_Logger.Error(string.Format("Invalid pair found in ExpiryOverride setting, invalid expiry timespan: '{0}'", text3));
					}
					else if (dictionary.ContainsKey(text2))
					{
						_Logger.Error(string.Format("Duplicate host '{0}' found in ExpiryOverride setting, ignoring", text2));
					}
					else
					{
			...
			{
				_Logger.Error(string.Format("Uncaught exception parsing ExpiryOverride setting! Returning incomplete results. Exception: {0}", arg));
			}
			return dictionary;
		}
		...
		private HashSet<SocketError> ParseSocketErrors(string csv)
		{
			...
			{
				_Logger.Error(string.Format("Uncaught exception parsing SocketErrors csv setting! Returning incomplete results. Exception: {0}", arg));
			}
			return hashSet;
		}
		...
		private HashSet<string> ParseExceptionNames(string csv)
		{
			try
			{
				return new HashSet<string>(from n in csv.Split(new char[1] { ',' })
			...
			{
				_Logger.Error(string.Format("Error while parsing exception names: '{0}' {1}", csv, arg));
		...
		private IPAddress[] ParseIpAddresses(string csv)
		{
			...
			try
			{
				...
				foreach (string text in array)
				{
					if (!string.IsNullOrWhiteSpace(text))
					{
						...
						{
							_Logger.Error(string.Format("Failed to parse IP Address {0}", text));
						}
					}
				}
			}
			...
			{
				_Logger.Error(string.Format("Uncaught exception parsing IPAddresses setting! Returning incomplete results. Exception: {0}", arg));
			}
			return list.ToArray();
		}
	}
