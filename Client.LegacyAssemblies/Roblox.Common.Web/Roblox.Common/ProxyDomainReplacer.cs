...
namespace Roblox.Common
{
	public class ProxyDomainReplacer
	{
		...
		private static bool EnableProxyUrlDetection
		{
			get
			{
				return Settings.Default.EnableProxyUrlDetection;
			}
		}
		...
		{
			string proxyHost;
			if (EnableProxyUrlDetection && IsProxiedChinaRequest(robloxCnProxyUrl, out proxyHost))
			{
		...
		public static bool IsProxiedChinaRequest(string robloxCnProxyUrl, out string proxyHost)
		{
			proxyHost = null;
			Uri proxyUrl;
			if (Uri.TryCreate(robloxCnProxyUrl, UriKind.Absolute, out proxyUrl))
			{
