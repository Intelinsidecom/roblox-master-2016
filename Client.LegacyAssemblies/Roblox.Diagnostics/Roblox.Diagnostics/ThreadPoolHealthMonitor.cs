			...
			while (_ContinueRunning)
			{
				try
				{
					...
					if (availableWorkerThreads <= _ThreadPoolExhaustionThresholdGetter())
					{
						...
						if (exhaustionDuration > _ThreadPoolExhaustionDurationGetter())
						{
							OnThreadPoolExhausted(availableWorkerThreads, availableCompletionPortThreads, exhaustionDuration);
						}
					}
					else if (_Exhausted.HasValue)
					{
						...
						_Exhausted = null;
						OnThreadPoolRecovered(availableWorkerThreads, availableCompletionPortThreads, exhaustionDuration2);
					}
