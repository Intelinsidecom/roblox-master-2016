...
CWebService::CWebService(bool crashUploadOnly) :
   ...
   dataModelCount(0)
{
	// Enable heap debugging but less aggressive to avoid thread issues
	_CrtSetDbgFlag(_CRTDBG_ALLOC_MEM_DF | _CRTDBG_LEAK_CHECK_DF);
	// Don't enable CHECK_ALWAYS_DF as it interferes with background threads
	...
	// Validate heap after analytics initialization
	if (_CrtCheckMemory() == FALSE) {
		RBX::StandardOut::singleton()->print(RBX::MESSAGE_ERROR, "Heap corruption detected after Analytics::InfluxDb::init()");
		RBXCRASH();
	}
	...
	// Validate heap after crash dump
	if (_CrtCheckMemory() == FALSE) {
		RBX::StandardOut::singleton()->print(RBX::MESSAGE_ERROR, "Heap corruption detected after WriteCrashDump()");
		RBXCRASH();
	}
	...
	// Validate heap after app settings
	if (_CrtCheckMemory() == FALSE) {
		RBX::StandardOut::singleton()->print(RBX::MESSAGE_ERROR, "Heap corruption detected after LoadAppSettings()");
		RBXCRASH();
	}
	...
	// Validate heap after client settings thread creation
	if (_CrtCheckMemory() == FALSE) {
		RBX::StandardOut::singleton()->print(RBX::MESSAGE_ERROR, "Heap corruption detected after validateClientSettings thread creation");
		RBXCRASH();
	}
	...
	// Validate heap after security updater creation
	if (_CrtCheckMemory() == FALSE) {
		RBX::StandardOut::singleton()->print(RBX::MESSAGE_ERROR, "Heap corruption detected after security updater creation");
		RBXCRASH();
	}
	...
	// Validate heap after MD5 updater run
	if (_CrtCheckMemory() == FALSE) {
		RBX::StandardOut::singleton()->print(RBX::MESSAGE_ERROR, "Heap corruption detected after MD5 updater run");
		RBXCRASH();
	}
