...
static const TCHAR* BootstrapperMutexName   = _T("www.freblx.xyz/bootstrapper");
static const TCHAR* StartRobloxAppMutex     = _T("www.freblx.xyz/startRobloxApp");
static const TCHAR* LauncherFileName        = _T("RobloxProxy.dll");
static const TCHAR* LauncherFileName64      = _T("RobloxProxy64.dll");
static const TCHAR* FriendlyName            = _T("Freebloxia");
static const TCHAR* CLSID_Launcher          = _T("{74AE169C-10C8-44FA-A11E-45485828B544}");
static const TCHAR* CLSID_Launcher64        = _T("{712239CB-EAA9-4B93-8DAA-83E015FAB987}");
static const TCHAR* AppID_Launcher          = _T("{C5B85B73-835E-4C60-AF94-5F281165C717}");
...
bool BootstrapperClient::NeedPreDeployRun()
{
	...
	CRegKey key;
	if (SUCCEEDED(key.Open(HKEY_CURRENT_USER, _T("Software\\Freebloxia Corporation\\Freebloxia"), KEY_READ)))
	{
		TCHAR buf[MAX_PATH];
...
void BootstrapperClient::RunPreDeploy()
{
	...
	try
	{
		...
		CRegKey key;
		if (SUCCEEDED(key.Create(HKEY_CURRENT_USER, _T("Software\\Freebloxia Corporation\\Freebloxia"))))
		{
...
bool BootstrapperClient::ProcessProtocolHandlerArgs(const std::map<std::wstring, std::wstring>& argMap)
{
	...
	if (launchMode == _T("play"))
	{
		...
		playArgs->authUrl = "http://" + BaseHost() + "/Login/Negotiate.ashx";
...
void BootstrapperClient::deployRobloxProxy(bool commitData)
{
...

	{
		...
		CreateKey(allowedDomainsKey, _T("freblx.xyz"));
...
void BootstrapperClient::registerFirefoxPlugin(const TCHAR* id, bool is64Bits)
{
	...
	key.SetStringValue(_T("Description"), _T("Freebloxia Launcher"));
	key.SetStringValue(_T("Vendor"), _T("Freebloxia"));
