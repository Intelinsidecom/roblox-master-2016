...
void deleteCurVersionKeys(simple_logger<wchar_t> &logger, bool isPerUser, const TCHAR *component)
{
	...
	if(!FAILED(key.Open(isPerUser ? HKEY_CURRENT_USER : HKEY_LOCAL_MACHINE, _T("Software\\Freebloxia Corporation\\Freebloxia"), KEY_WRITE)))
	{
...
void setCurrentVersion(simple_logger<wchar_t> &logger, bool isPerUser, const TCHAR* componentCode, const TCHAR* version, const TCHAR* baseUrl)
{
	...
	if (ERROR_SUCCESS == key.Create(isPerUser ? HKEY_CURRENT_USER : HKEY_LOCAL_MACHINE, _T("Software\\Freebloxia Corporation\\Freebloxia")))
	{
...
void getCurrentVersion(simple_logger<wchar_t> &logger, bool isPerUser, const TCHAR* componentCode, TCHAR* version, int vBufSize, TCHAR* baseUrl, int uBaseSize)
{
	...
	version[0] = 0;
	baseUrl[0] = 0;
	if (ERROR_SUCCESS == key.Open(isPerUser ? HKEY_CURRENT_USER : HKEY_LOCAL_MACHINE, _T("Software\\Freebloxia Corporation\\Freebloxia"), KEY_READ))
	{
		std::wstring vKey = buildVersionKey(componentCode);
		std::wstring uKey = buildUrlKey(componentCode);
...
{
	std::wstring scheme = _T("freblx-player");
...
{
	std::wstring scheme = _T("freblx-studio");
...
{
	return _T("StudioQTFreebloxiaReg");
}
