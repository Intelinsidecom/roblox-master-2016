...
DeviceContextD3D9::DeviceContextD3D9(Device* device)
    ...
    , d3d9(NULL)
{
    // Xbox 360: debug markers are provided by the Pix* APIs directly (see below); no d3d9.dll loading.
    pfn_D3DPERF_BeginEvent = 0;
    pfn_D3DPERF_EndEvent = 0;
    pfn_D3DPERF_SetMarker = 0;

    // Markers are always available via PIX on Xbox 360.
}

DeviceContextD3D9::~DeviceContextD3D9()
{
    // Xbox 360: no d3d9.dll was loaded.
}
...
void DeviceContextD3D9::resolveFramebuffer(Framebuffer* msaaBuffer, Framebuffer* buffer, unsigned int mask)
{
    ...
    // Xbox 360: MSAA is not exposed (maxSamples = 1), so this blit path is unused.
    // If MSAA is ever enabled, resolve via D3DDevice::Resolve into the destination texture.
    (void)msaaBufferColor9;
    (void)bufferColor9;
}
...
{
    // Xbox 360 has no fixed-function pipeline; these transforms are unused.
    (void)data;
    (void)dataSize;
}
...
{
    // No fixed-function texture factor on Xbox 360.
    (void)handle;
    (void)data;
    (void)vectorCount;
}
