...
{
	{ D3DPOOL_MANAGED, 0 },
	{ D3DPOOL_DEFAULT, D3DUSAGE_CPU_CACHED_MEMORY },
...
void TextureD3D9::commitChanges()
{
	...
	if (mirror && mirrorDirty)
	{
        ...
        // Xbox 360: no UpdateTexture; use LockRect + memcpy with tiling conversion
        // TODO: use XGTileTextureLevel for correct linear→tiled transfer
        D3DTexture* dstTex = static_cast<D3DTexture*>(object);
        D3DTexture* srcTex = static_cast<D3DTexture*>(mirror);
        D3DLOCKED_RECT dstRect, srcRect;
        if (SUCCEEDED(dstTex->LockRect(0, &dstRect, NULL, 0)) && SUCCEEDED(srcTex->LockRect(0, &srcRect, NULL, D3DLOCK_READONLY)))
        {
            memcpy(dstRect.pBits, srcRect.pBits, dstRect.Pitch * height);
            srcTex->UnlockRect(0);
            dstTex->UnlockRect(0);
        }
...
void TextureD3D9::generateMipmaps()
{
    ...
    // Xbox 360: no StretchRect for mipmap generation; mipmaps should be generated
    // offline or via GPU shader. This is a no-op on 360 for bring-up.
    // TODO: implement mipmap generation via shader or XG mipmap auto-generation.
}
...
void TextureD3D9::markAsDirty()
{
	if (mirror)
	{
        // Xbox 360: no UpdateTexture; defer to commitChanges/flushChanges with LockRect path
        mirrorDirty = true;
	}
}
