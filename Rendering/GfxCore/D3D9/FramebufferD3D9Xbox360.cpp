...
IDirect3DSurface9* FramebufferD3D9::grabCopy()
{
    ...
    // Xbox 360: no CreateOffscreenPlainSurface/GetRenderTargetData; use Resolve into a system-memory texture
    // TODO: proper lifecycle management for the resolved texture; currently leaks the texture.
    IDirect3DTexture9* systemTexture = NULL;
    HRESULT hr = device9->CreateTexture(desc.Width, desc.Height, 1, D3DUSAGE_RENDERTARGET, desc.Format, D3DPOOL_SYSTEMMEM, &systemTexture, NULL);
    if (FAILED(hr))
        throw RBX::runtime_error("grabCopy failed: can't create system-memory texture: %x", hr);

    IDirect3DSurface9* systemSurface = NULL;
    hr = systemTexture->GetSurfaceLevel(0, &systemSurface);
    if (FAILED(hr))
    {
        systemTexture->Release();
        throw RBX::runtime_error("grabCopy failed: can't get surface level: %x", hr);
    }

    hr = device9->Resolve(0, NULL, systemTexture, NULL, 0, 0, NULL, 0, 0, NULL);
    if (FAILED(hr))
    {
        systemSurface->Release();
        systemTexture->Release();
        throw RBX::runtime_error("grabCopy failed: resolve failed: %x", hr);
    }

    // Note: systemTexture is leaked here (caller releases systemSurface only).
    // For bring-up this works; needs proper ownership tracking.
