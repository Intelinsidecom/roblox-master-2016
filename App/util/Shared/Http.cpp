...
namespace
{
#if defined(RBX_PLATFORM_DURANGO) || defined(RBX_PLATFORM_UWP) || defined(RBX_PLATFORM_XBOX360) || defined(RBX_PLATFORM_WIN_PHONE)
					   ...
					   bool forceNativeHttp)
{
	...
	if (!useCurlHttpImpl || forceNativeHttp)
	{
		try
		{
			httpGetPostWinRT(isPost, dataStream, contentType, compressData, headers, externalRequest, cachePolicy, response);
			if (recordStatistics)
			{
				HTTPStatistics::success(httpTimer.delta().msec(), url.c_str(), response.size());
			}
