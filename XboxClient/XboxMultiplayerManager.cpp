...
XboxMultiplayerManager::~XboxMultiplayerManager()
{
	...
	stopPolling = true;
	if (sessionPollingThread && sessionPollingThread->joinable())
	{
		sessionPollingThread->join();
	}
#endif
...
#if USE_XDK_2015_RTA
	// XDK 2015: Use RealTimeActivity subscription APIs
...
#else
	// XDK 2016+: Use polling-based approach since RTA APIs were removed
	stopPolling = false;
	sessionPollingThread.reset(new std::thread([this]()
	{
		dprintf("XboxMultiplayerManager: Session polling thread started\n");
		
		while (!stopPolling)
		{
			XboxLiveContext^ xblContext = nullptr;
			MultiplayerSession^ currentSession = nullptr;
			unsigned long long knownChange = 0;
			
			{
				RBX::mutex::scoped_lock lock(stateLock);
				xblContext = xboxLiveContext;
				currentSession = gameSession;
				knownChange = lastKnownChangeNumber;
			}
			
			if (xblContext && currentSession)
			{
				try
				{
					// Get latest session state
					auto sessionRef = currentSession->SessionReference;
					auto task = xblContext->MultiplayerService->GetCurrentSessionAsync(sessionRef);
					
					task->Completed = ref new AsyncOperationCompletedHandler<MultiplayerSession^>(
						[this, knownChange](IAsyncOperation<MultiplayerSession^>^ op, AsyncStatus status)
					{
						if (status == AsyncStatus::Completed)
						{
							MultiplayerSession^ updatedSession = op->GetResults();
							if (updatedSession && updatedSession->ChangeNumber > knownChange)
							{
								dprintf("XboxMultiplayerManager: Session change detected (change %llu -> %llu)\n", 
									knownChange, updatedSession->ChangeNumber);
								
								RBX::mutex::scoped_lock lock(taskLock);
								processSessionChange(updatedSession);
								
								RBX::mutex::scoped_lock stateLockLocal(stateLock);
								lastKnownChangeNumber = updatedSession->ChangeNumber;
							}
						}
					});
				}
				...
				{
					dprintf("XboxMultiplayerManager: Polling exception [0x%x]: %S\n", 
						(int)e->HResult, e->Message->Data());
				}
			}
			
			// Sleep for 500ms between polls
			for (int i = 0; i < 5 && !stopPolling; i++)
			{
				Sleep(100);
			}
		}
		
		dprintf("XboxMultiplayerManager: Session polling thread stopped\n");
	}));
#endif
}
...
#if USE_XDK_2015_RTA
	// XDK 2015: Unsubscribe from RTA events
	if (xboxLiveContext->RealTimeActivityService->MultiplayerSubscriptionsEnabled)
	{
...
#else
	// XDK 2016+: Stop the polling thread
	stopPolling = true;
	if (sessionPollingThread && sessionPollingThread->joinable())
	{
		sessionPollingThread->join();
		sessionPollingThread.reset();
	}
#endif
}
...
#else
			// XDK 2016+: WriteSessionResult->Details removed, use status-based message
			throw ref new Platform::FailureException("Failed to join game session (Status: " + result->Status.ToString() + ")");
#endif
		}
...
#else
			// XDK 2016+: WriteSessionResult->Details removed, use status-based message
			throw ref new Platform::FailureException("Failed to join party session (Status: " + result->Status.ToString() + ")");
#endif
		}
...
#else
			// XDK 2016+: WriteSessionResult->Details removed, use status-based message
			throw ref new Platform::FailureException("Failed to join multiplayer session (Status: " + result->Status.ToString() + ")");
#endif
		}
...
#else
			// XDK 2016+: Party API changed significantly
			// The ShowSendGameInvitesAsync signature changed to require different parameters
			// For 2016+ XDK, invitations are typically handled through Activity system
			// or via the newer SocialManager/GameInvite APIs
			// TODO: Implement proper invite flow for XDK 2016+
			dprintf("Party invites not implemented for XDK 2016+ - use game session invites instead\n");
#endif
