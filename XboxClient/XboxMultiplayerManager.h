...
// Toggle between XDK 2015 RTA APIs (1) and XDK 2016+ polling approach (0)
// Set to 1 only if building with 2015 XDK
...
// XDK 2016+ compatibility: Need these for polling-based approach
#if !USE_XDK_2015_RTA
#include <thread>
#include <atomic>
#include <memory>
...
#if !USE_XDK_2015_RTA
	// XDK 2016+: Polling-based session monitoring (RTA APIs removed)
	std::unique_ptr<std::thread> sessionPollingThread;
	std::atomic<bool> stopPolling;
	std::atomic<unsigned long long> lastKnownChangeNumber;
#endif
...
#ifdef USE_XDK_2015_RTA
	void onSessionChanged( Platform::Object^ object, RealTimeActivity::RealTimeActivityMultiplayerSessionChangeEventArgs^ arg );
#else
	void onSessionChanged(Platform::Object^ object, Multiplayer::MultiplayerSessionChangeEventArgs^ arg);
#endif
